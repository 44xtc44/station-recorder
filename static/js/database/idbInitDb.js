...
/**
 * This guy can create a DB and object stores, single or batch mode.
 ...
 */
function createIndexedDb(kwargs) {
  // Promise to schedule app state update fun.
  const dbName = kwargs.dbName;
  const dbVersion = kwargs.dbVersion;
  const storeName = kwargs.storeName;
  const primaryKey = kwargs.primaryKey;
  const indexNames = kwargs.indexNames;
  const objStores = kwargs.objStores;
  ...
  return new Promise((resolve, _) => {
    ...
    open.onupgradeneeded = function (event) {
      ...
      if (kwargs.batchCreate === undefined || !kwargs.batchCreate) {

...
async function homePageOpen() {
  ...
  const userDataDir = ""; // "" means temporary directory elsewhere -> Docker container write!!!
  const browserContext = await chromium.launchPersistentContext(userDataDir, {
    channel: "chromium",
    headless: false,
    args: [
      `--disable-extensions-except=${pathToExtension}`, // no error message if comment out
      `--load-extension=${pathToExtension}`,
      `--allow-file-access-from-files`, // extension reads files from its app folder
 ...
 *  npx run-func chromium.js homePageOpen -y // yes
 * 
 * manifest.json in test folder needs path to start script
 *   "background": {
